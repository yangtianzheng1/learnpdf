...
import com.example.pdflibrary.view.PagePart;
...
public class RenderingHandler extends Handler {
    ...
    private PagePart proceed(RenderingTask renderingTask) throws PageRenderingException {
        ...
        LogUtils.logD(TAG, " width  " + w + " height " + h);
        LogUtils.logD(TAG, " renderingTask.bounds " + renderingTask.bounds.toString());

        calculateBounds(w, h, renderingTask.bounds, renderingTask.thumbnail);

        pdfFile.renderPageBitmap(render, renderingTask.page, roundedRenderBounds, renderingTask.annotationRendering);

        LogUtils.logD(TAG, " roundedRenderBounds " + roundedRenderBounds.toString());
    ...
    private void calculateBounds(int width, int height, RectF pageSliceBounds, boolean isThumb) {
        renderMatrix.reset();
        renderMatrix.postTranslate(-pageSliceBounds.left * width, -pageSliceBounds.top * height);
        LogUtils.logD(TAG, " transX " + (-pageSliceBounds.left * width) + " transY " + (-pageSliceBounds.top * height));
        renderMatrix.postScale(1 / pageSliceBounds.width(), 1 / pageSliceBounds.height());
        LogUtils.logD(TAG, " scaleX " + (1 / pageSliceBounds.width()) + " scaleY " + (1 / pageSliceBounds.height()));

        renderBounds.set(0, 0, width, height);
        renderMatrix.mapRect(renderBounds);
//        Log.d(TAG, " renderBounds " + renderBounds.toString());
