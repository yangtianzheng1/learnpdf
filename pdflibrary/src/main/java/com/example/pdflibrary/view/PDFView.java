...
import com.example.pdflibrary.link.DefaultLinkHandler;
import com.example.pdflibrary.link.LinkHandler;
import com.example.pdflibrary.listener.Callbacks;
...
import com.example.pdflibrary.listener.OnLongPressListener;
...
import com.example.pdflibrary.listener.OnRenderListener;
import com.example.pdflibrary.listener.OnTapListener;
import com.example.pdflibrary.manager.AnimationManager;
import com.example.pdflibrary.manager.CacheManager;
import com.example.pdflibrary.manager.DecodingAsyncTask;
import com.example.pdflibrary.manager.DragPinchManager;
import com.example.pdflibrary.manager.PagesLoader;
import com.example.pdflibrary.manager.RenderingHandler;
import com.example.pdflibrary.scroll.ScrollHandle;
import com.example.pdflibrary.source.AssetSource;
import com.example.pdflibrary.source.ByteArraySource;
import com.example.pdflibrary.source.DocumentSource;
import com.example.pdflibrary.source.FileSource;
import com.example.pdflibrary.source.InputStreamSource;
import com.example.pdflibrary.source.UriSource;
import com.example.pdflibrary.util.DPTOPXUtil;
import com.example.pdflibrary.util.LogUtils;
import com.example.pdflibrary.util.MathUtils;
...
public class PDFView extends RelativeLayout {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (PagePart part : cacheManager.getThumbnails()) {
            LogUtils.logD(TAG, "Draws thumbnails");
    ...
    private void drawPart(Canvas canvas, PagePart part) {
        ...
        LogUtils.logD(TAG, " pageRelativeBounds " + pageRelativeBounds);
    ...
    public void performPageSnap() {
        ...
        Constants.SnapEdge edge = findSnapEdge(centerPage);
        if (edge == Constants.SnapEdge.NONE) {
            return;
        }
     ...
     */
    public Constants.SnapEdge findSnapEdge(int page) {
        if (!pageSnap || page < 0) {
            return Constants.SnapEdge.NONE;
        }
        ...
        if (length >= pageLength) {
            return Constants.SnapEdge.CENTER;
        } else if (currentOffset >= offset) {
            return Constants.SnapEdge.START;
        } else if (offset - pageLength > currentOffset - length) {
            return Constants.SnapEdge.END;
        } else {
            return Constants.SnapEdge.NONE;
        }
    }
     ...
     */
    public float snapOffsetForPage(int pageIndex, Constants.SnapEdge edge) {
        ...
        if (edge == Constants.SnapEdge.CENTER) {
            offset = offset - length / 2f + pageLength / 2f;
        } else if (edge == Constants.SnapEdge.END) {
    ...
    public void fitToWidth(int page) {
        if (state != State.SHOWN) {
            LogUtils.logE(TAG, "Cannot fit, document not rendered yet");
            return;
        }
    ...
    public float getCurrentXOffset() {
        LogUtils.logD(TAG, " currentXOffset " + currentXOffset);
        return currentXOffset;
    }

    public float getCurrentYOffset() {
        LogUtils.logD(TAG, " currentYOffset " + currentYOffset);
    ...
    private void setPageFitPolicy(Constants.FitPolicy pageFitPolicy) {
        this.pageFitPolicy = pageFitPolicy;
    }

    public Constants.FitPolicy getPageFitPolicy() {
    ...
    public class Configurator {
        ...
        public Configurator pageFitPolicy(Constants.FitPolicy pageFitPolicy) {
            this.pageFitPolicy = pageFitPolicy;
            return this;
        }
